...
For Python symbols, references, call sites, and import edges, query the syntax tree with `~/copilot/bin/pysymbols.py`. It reads the AST, so it will not match a name that appears only in a comment, a docstring, or an unrelated language.
...
skips .venv, __pycache__, and node_modules, reports unparseable files on

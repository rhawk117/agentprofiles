            ...
            if isinstance(node, ast.ClassDef) and node.name == name:
                ...
                hits.append(
                    (
                        ...
                        node.lineno,
                        kind,
                        f"{signature(node)}{decorators(node)}",
                    )
                )
                ...
                if node.target.id == name:
                    hits.append(
                        (
                            ...
                            node.lineno,
                            "annassign",
                            f"{name}: {ast.unparse(node.annotation)}",
                        )
                    )
    return hits
    ...
    for path in sources(target):
        ...
        for node in ast.walk(tree):
            if calls_only:
                ...
                if matched:
                    hits.append(
                        (str(path), node.lineno, "call", ast.unparse(node)[:80])
                    )
                continue
            if isinstance(node, (ast.Import, ast.ImportFrom)):
                for alias in node.names:
                    if (alias.asname or alias.name.split(".")[0]) == name:
                        hits.append(
                            (str(path), node.lineno, "import", ast.unparse(node)[:80])
                        )
    ...
    for path in sources(target):
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                ...
                hits.append(
                    (str(path), node.lineno, "from", f"{module} import {names}")
                )
    return hits
...
def find_importers(module: str, target: Path) -> list[Hit]:
    ...
    for path, line, kind, detail in find_imports(target):
        head = (
            detail.split(" import ")[0].split(" as ")[0].strip()
            if kind == "from"
            else detail.split(" as ")[0]
        )
            ...
            if isinstance(node, ast.ClassDef):
                ...
                hits.append(
                    (
                        ...
                        node.lineno,
                        "class",
                        f"{prefix}{node.name}{suffix}{decorators(node)}",
                    )
                )
                ...
                hits.append(
                    (
                        ...
                        node.lineno,
                        kind,
                        f"{prefix}{signature(node)}{decorators(node)}",
                    )
                )
